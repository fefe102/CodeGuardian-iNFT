...
describe("explorer app smoke tests", () => {
  ...
  it("CodeGuardian page renders high-tier verification", async () => {
    const html = renderToString(await AgentPage({ params: Promise.resolve({ agent: "codeguardian" }) }));
  ...
  it("FakeAgent page renders failures", async () => {
    const html = renderToString(await AgentPage({ params: Promise.resolve({ agent: "fakeagent" }) }));
  ...
  it("replay page renders the timeline", async () => {
    const html = renderToString(await RunPage({ params: Promise.resolve({ runId: "codeguardian-run-001" }) }));
    expect(html).toContain("task_received");
    expect(html).toContain("certificate_issued");
  });

  it("certificate page renders certificate", async () => {
    const html = renderToString(await CertificatePage({ params: Promise.resolve({ certificateId: "poi-cert-codeguardian-001" }) }));
...
describe("admin route security", () => {
  ...
  it("rejects an invalid token", () => {
    process.env[adminTokenName] = "fixture-admin-token";
    process.env.POI_ENABLE_LIVE_WRITES = "true";
    const result = validateAdminRequest({ headers: new Headers({ authorization: "Bearer wrong" }) });
    expect(result?.status).toBe(401);
  });
