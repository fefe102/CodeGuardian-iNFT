...
export function validateAdminRequest(request: Pick<NextRequest, "headers">): AdminResult | null {
  ...
  if (!configuredToken) {
    return {
      ...
      body: {
        ok: false,
        error: "Live write actions are disabled. Public verification remains available."
      }
    };
  }
  ...
  if (!supplied) {
    return { ok: false, status: 401, body: { ok: false, error: "Missing admin token" } };
  }

  if (supplied !== configuredToken) {
    return { ok: false, status: 401, body: { ok: false, error: "Invalid admin token" } };
  }
