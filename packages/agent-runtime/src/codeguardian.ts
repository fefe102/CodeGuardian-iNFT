...
export class MockCodeGuardianCompute {
  analyze(source: string) {
    const hasUnsafeParse = source.includes("JSON.parse") && source.includes("as Result");
    return {
      ...
      outputHash: hashCanonicalJson({ issue: hasUnsafeParse, patch: "validate-result-shape" })
    };
  }

  critique(issue: string, patch: string) {
    return {
      critique: "The patch is bounded, keeps the public API stable, and makes the unsafe cast explicit.",
      promptHash: hashCanonicalJson({ task: "critic", issue, patch }),
      outputHash: hashCanonicalJson({ accepted: true, critique: "bounded-shape-validation" }),
      accepted: true
    };
  }
}
...
export class InMemoryStateStore {
  ...
  root() {
    return hashCanonicalJson(Object.fromEntries([...this.values.entries()].sort(([left], [right]) => left.localeCompare(right))));
  }
}
...
export class CodeGuardian {
  ...
  run(options: CodeGuardianOptions = {}): CodeGuardianRunResult {
    ...
    const source = options.targetSource ?? readFixtureSource(options.targetPath);
    const mode = options.source ?? "hybrid";
    const memoryRootBefore = this.state.root();
    const baseAt = Date.parse("2026-04-26T00:00:00.000Z");
    const at = (seconds: number) => new Date(baseAt + seconds * 1000).toISOString();

    const analysis = this.compute.analyze(source);
    const critic = this.compute.critique(analysis.issue, analysis.patch);

    this.append("task_received", at(0), { target: options.targetPath ?? "examples/codeguardian/fixtures/unsafe-parser.ts" });
    this.append("compute_started", at(1), { runId: "zg-compute-analysis-001", model: "qwen/qwen-2.5-7b-instruct" });
    this.append("compute_completed", at(4), { outputHash: analysis.outputHash });
    this.append("issue_found", at(5), { issue: analysis.issue });
    this.append("patch_proposed", at(7), { patch: analysis.patch });
    this.append("critic_started", at(8), { runId: "zg-compute-critic-001" });
    this.append("critic_completed", at(10), { critique: critic.critique, accepted: critic.accepted });
    ...
    const memory: MemoryState = {
      ...
      checkpoint: {
        ...
        stateRoot: hashCanonicalJson({ issue: analysis.issue, patch: analysis.patch, critique: critic.critique })
      },
      history: [
        {
          runId,
    ...
    this.append("certificate_issued", at(13), { certificateId: "poi-cert-codeguardian-001" });
    ...
    const computeRuns: ComputeRuns = {
      ...
      provider: "0G Compute mock/hybrid",
      model: "qwen/qwen-2.5-7b-instruct",
      runs: [
        {
          id: "zg-compute-analysis-001",
          type: "analysis",
    ...
    const certificate: Certificate = {
      ...
      evidence: {
        ...
        intelligenceBundleRoot: "sha256:1c47d70c2c89e0d843c1b8501e98f5bd2354f7b03b994e1b819b2fbc54b87fd4",
    ...
    return { run, memory, computeRuns, certificate, memoryRootBefore, memoryRootAfter };
  }
...
export function replayCodeGuardianRun(run: RunTrace) {
  return {
    ...
    timeline: run.events.map((event, index) => ({ index, type: event.type, at: event.at, detail: event.detail })),
    root: hashCanonicalJson(run)
  };
}

export function writeCodeGuardianArtifacts(result: CodeGuardianRunResult, directory = "tmp/codeguardian") {
...
function readFixtureSource(path = "examples/codeguardian/fixtures/unsafe-parser.ts") {
  if (existsSync(path)) {
